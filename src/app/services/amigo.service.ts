...
export class AmigoService {
  ...
  private readonly AMISTADES_BASE =
    'https://mikhuy-backend.onrender.com/api/amistades';
  ...
  getCompaneros(): Observable<Companero[]> {
    return this.http
      .get<{
        success: boolean;
        data: Companero[];
      }>(`${this.BASE}/companeros`, { headers: this.authHeaders() })
      .pipe(map((r) => r.data ?? []));
  }
  ...
  getAmigos(): Observable<Companero[]> {
    return this.http
      .get<{
        success: boolean;
        data: Companero[];
      }>(`${this.AMISTADES_BASE}`, { headers: this.authHeaders() })
  ...
  getSolicitudesRecibidas(): Observable<any[]> {
    return this.http
      .get<{
        success: boolean;
        data: any[];
      }>(`${this.AMISTADES_BASE}/solicitudes-recibidas`, { headers: this.authHeaders() })
      .pipe(map((res) => res.data || []));
  }
  ...
  getSolicitudesEnviadas(): Observable<string[]> {
    return this.http
      .get<{
        success: boolean;
        data: string[];
      }>(`${this.AMISTADES_BASE}/solicitudes-enviadas`, { headers: this.authHeaders() })
      .pipe(map((r) => r.data ?? []));
  }
  ...
  return this.http
    ...
    .pipe(
      map(res => {
        let estado: string;
        if (typeof res.data === 'string') estado = res.data;
        else if (res.data && res.data.estado) estado = res.data.estado;
        else estado = 'ninguno';
        return estado as EstadoAmistad;
      })
    );
}
  ...
  eliminarRelacion(otroId: string): Observable<void> {
    return this.http.delete<void>(`${this.AMISTADES_BASE}/${otroId}`, {
      headers: this.authHeaders(),
    });
  }
  ...
  notificar(payload: {
    ...
    return this.http.post<void>(`${this.BASE}/notificar-amigo`, payload, {
      headers: this.authHeaders(),
    });
  }
