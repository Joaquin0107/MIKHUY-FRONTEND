...
import { ApiResponse } from '../../services/juego.service';
...
export class JuegosComponent implements OnInit {
  ...
  loadJuegos(): void {
    this.loading = true;
    this.studentPoints = this.calcularPuntosTotales(this.juegos);
    localStorage.setItem('studentPoints', this.studentPoints.toString());
    this.juegosService.getMisJuegos().subscribe({
      next: (response) => {
        if (response.success) {
          ...
          this.studentPoints = this.juegosService.getTotalPuntos(this.juegos);
          localStorage.setItem('studentPoints', this.studentPoints.toString());
  ...
  calcularPuntosTotales(juegos: JuegoResponse[]): number {
    return juegos.reduce(
      (total, juego) => total + (juego.puntosGanados || 0),
      0
    );
  }
          ...
          <mat-card-content>
            ...
            <div class="level-info">
              ...
              <p>
                ...
                {{ gameData.juego.puntosGanados }}
              </p>
              <p>
                <strong>Puntos por nivel:</strong>
                {{ gameData.juego.puntosPorNivel }}
              </p>
            </div>
          </mat-card-content>
          <mat-card-actions>
      ...
      <div class="game-content" *ngIf="gameStarted && !gameEnded">
          ...
          <mat-card class="question-card">
            ...
            <mat-card-content>
              ...
              >
                <mat-radio-button
                  *ngFor="
                    let opcion of preguntaNutrimental?.opciones;
                    let i = index
                  "
                  [value]="i"
              ...
              <div
                *ngIf="respuestaEnviada"
                class="feedback"
                [class.correct]="esRespuestaCorrecta"
                [class.incorrect]="!esRespuestaCorrecta"
              >
                <mat-icon>{{
                ...
                <p class="explanation">
                  {{ preguntaNutrimental?.explicacion }}
                </p>
              </div>
            </mat-card-content>
            <mat-card-actions>
            ...
            <mat-card-content>
              ...
              <div class="food-groups">
                ...
                <div class="food-item">
                  ...
                  <input
                    type="number"
                    [(ngModel)]="alimentosCarbohidratos"
                    min="0"
                    max="10"
                  />
                </div>
              ...
              <div class="emotions">
                ...
                <mat-radio-group [(ngModel)]="emocionSeleccionada">
                  ...
                  <mat-radio-button value="estresado"
                    >😰 Estresado</mat-radio-button
                  >
                  <mat-radio-button value="ansioso"
                    >😟 Ansioso</mat-radio-button
                  >
                </mat-radio-group>
              </div>
        ...
        <div *ngIf="esCoachExpres()" class="coach-game">
          <mat-card class="coach-card">
            <mat-card-header>
              <mat-card-title>
                Pregunta {{ preguntaCoachActual + 1 }} de
                {{ totalPreguntasCoach }}
              </mat-card-title>
            ...
            <mat-card-actions>
              ...
              >
                {{
                  preguntaCoachActual < totalPreguntasCoach - 1
                    ? 'Siguiente'
                    : 'Finalizar'
                }}
              </button>
...
export class GamePlayDialog implements OnInit, OnDestroy {
  ...
  preguntasNutrimental: any[] = [
    {
      ...
      tema: 'Vitaminas',
      explicacion:
        'La vitamina A es esencial para mantener una buena visión, especialmente en condiciones de poca luz.',
    },
    {
      numero: 2,
    ...
    },
    {
      ...
      tema: 'Hidratación',
      explicacion:
        'Se recomienda beber entre 6-8 vasos de agua al día para mantenerse bien hidratado.',
    },
    {
      numero: 4,
      ...
      tema: 'Minerales',
      explicacion:
        'El calcio es fundamental para mantener huesos y dientes fuertes.',
    },
    {
      numero: 5,
      ...
      tema: 'Proteínas',
      explicacion:
        'El pollo es una excelente fuente de proteína magra, esencial para el crecimiento y reparación de tejidos.',
    },
  ];
  ...
  preguntasCoach: any[] = [
    ...
    },
    {
      numero: 4,
      pregunta:
        '¿Has intentado mejorar tus hábitos alimenticios recientemente?',
      etapa: 'Acción',
    },
    {
      numero: 5,
      pregunta:
        '¿Te sientes capaz de mantener una alimentación saludable a largo plazo?',
      etapa: 'Mantenimiento',
    },
    {
      numero: 6,
    ...
    },
    {
      numero: 7,
      pregunta:
        '¿Conoces los beneficios de una alimentación balanceada para tu salud?',
      etapa: 'Conocimiento',
    },
    {
      numero: 8,
  ...
  esNutrimental(): boolean {
    return (
      this.gameData.juego.nombre.toLowerCase().includes('nutrimental') ||
  ...
  esReto7Dias(): boolean {
    return (
      this.gameData.juego.nombre.toLowerCase().includes('reto') ||
  ...
  enviarRespuestaNutrimental(): void {
    ...
    const tiempoRespuesta =
      this.tiempoTranscurrido - this.tiempoInicioRespuesta;
    ...
    this.sesionService
      .guardarRegistroReto7Dias({
        ...
        emocion: this.emocionSeleccionada
          ? (this.emocionSeleccionada as any)
          : undefined,
        ...
        next: (response) => {
          if (response.success) {
            ...
            const puntosActuales = parseInt(
              localStorage.getItem('studentPoints') || '0'
            );
            localStorage.setItem(
              'studentPoints',
              (puntosActuales + this.puntosGanadosEnSesion).toString()
            );
          }
          this.loading = false;
        },
  ...
  abandonar(): void {
    if (this.gameStarted && !this.gameEnded) {
      const confirmar = confirm(
        '¿Estás seguro de que quieres abandonar? Se perderá tu progreso.'
      );
      if (!confirmar) return;
...
@Component({
  ...
  styles: [
    `
      .badge-accent .mat-badge-content {
        background-color: #ff4081 !important; /* color 'accent' */
      }
      .badge-warn .mat-badge-content {
        background-color: #f44336 !important; /* color 'warn' */
      }
      .mi-posicion {
        margin-top: 20px;
        padding-top: 10px;
