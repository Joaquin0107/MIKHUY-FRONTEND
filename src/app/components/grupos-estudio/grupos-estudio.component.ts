...
export class GruposEstudioComponent implements OnInit {
  ...
  cargarEstudiantes(): void {
    this.http.get<any>(`${this.estudiantesUrl}`).subscribe({
      next: (res) => { this.estudiantesDisponibles = res.data || []; },
      error: () => {}
    });
  }
  ...
  toggleEstudiante(id: string): void {
    ...
    if (idx >= 0) {
      ...
      if (this.estudiantesSeleccionados.length >= 5) {
        this.snackBar.open('Máximo 5 estudiantes por grupo', 'OK', { duration: 3000 });
        return;
      }
  ...
  guardarGrupo(): void {
    if (!this.nombreGrupo.trim()) {
      this.snackBar.open('El nombre del grupo es obligatorio', 'OK', { duration: 3000 });
      return;
    }
    if (this.estudiantesSeleccionados.length === 0) {
      this.snackBar.open('Selecciona al menos un estudiante', 'OK', { duration: 3000 });
      return;
    }
    ...
    req.subscribe({
      next: () => {
        this.snackBar.open(
          this.modoEdicion ? 'Grupo actualizado' : 'Grupo creado', 'OK', { duration: 3000 });
        this.cerrarModal();
        this.cargarGrupos();
      },
      error: (err) => {
        this.snackBar.open(err.error?.message || 'Error guardando grupo', 'OK', { duration: 3000 });
      }
    });
  }
  ...
  eliminarGrupo(id: string): void {
    ...
    this.http.delete<any>(`${this.apiUrl}/${id}`).subscribe({
      ...
      error: () => this.snackBar.open('Error eliminando grupo', 'OK', { duration: 3000 })
    });
  }
  ...
  getMedalColor(pos: number): string {
    return pos === 1 ? '#FFD700' : pos === 2 ? '#C0C0C0' : pos === 3 ? '#CD7F32' : '#90a4ae';
  }
